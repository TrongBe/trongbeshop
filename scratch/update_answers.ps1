$content = Get-Content 'c:\study\trongbeshop\trongbeshop\js\data.js' -Raw
$answers = @{
...
foreach ($key in $answers.Keys) {
  $val = $answers[$key]
  $content = [regex]::Replace($content, "(?s)(id:\s*`"$key`".*?correctIndex:\s*)\d+(.*?})", "`${1}$val`${2}")
}
[IO.File]::WriteAllText('c:\study\trongbeshop\trongbeshop\js\data.js', $content, [System.Text.Encoding]::UTF8)
Write-Output "Done"
